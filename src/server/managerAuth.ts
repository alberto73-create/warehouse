...
export function managerEnvironment(){return {pin:Boolean(process.env.MANAGER_PIN),tokenSecret:Boolean(secret())}}
export function verifyManagerPin(pin:unknown){const configured=process.env.MANAGER_PIN;if(typeof pin!=='string'||!configured)return false;const supplied=Buffer.from(pin),expected=Buffer.from(configured);return supplied.length===expected.length&&timingSafeEqual(supplied,expected)}
...
export const managerCookieName='warehouse-manager';
export function managerTokenFromRequest(request:Request){const raw=request.headers.get('cookie')??'';const value=raw.split(';').map(item=>item.trim()).find(item=>item.startsWith(`${managerCookieName}=`));return value?decodeURIComponent(value.slice(managerCookieName.length+1)):undefined}
