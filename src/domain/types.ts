...
export interface Session { id:'current'; operator:string; role:'base'|'manager'; managerToken?:string }
export interface LocationConfig {id:Exclude<Place,`buoni-${number}`>;label:string;kind:'physical'|'logistic'|'request'|'external';updatedAt?:string;updatedBy?:string}
