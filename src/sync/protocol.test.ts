...
import {defaultConfiguration,defaultLocations} from './catalog';
...
const request=(deviceId:string,cursor:number,parts:Part[],operations:Movement[]=[])=>({deviceId,cursor,operations,catalog:{parts,bins:demoBins,locations:defaultLocations,configuration:defaultConfiguration}});
