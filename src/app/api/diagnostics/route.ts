...
import {managerEnvironment,managerTokenFromRequest,verifyManagerToken} from '@/server/managerAuth';
...
export async function GET(request:Request){
 ...
 const adapter=process.env.DATA_ADAPTER??'mock',manager=managerEnvironment(),environment={clientEmail:Boolean(process.env.GOOGLE_CLIENT_EMAIL),privateKey:Boolean(process.env.GOOGLE_PRIVATE_KEY),sheetId:Boolean(process.env.GOOGLE_SHEET_ID),managerPin:manager.pin,managerTokenSecret:manager.tokenSecret};
